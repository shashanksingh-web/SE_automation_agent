...
# possible route"). Admin Control Panel-overridable, same module-attr mechanism as the
# Routing ceilings above -- see planning.admin_config's "Plan C (AI-Reasoned)" group.
# Only ever changes which guidance sentence build_route_llm_reasoned puts in its
...
PLAN_C_DECISION_STYLE_GUIDANCE: Dict[str, str] = {
    ...
    "priority_focused": (
        "Prioritize maximizing total Priority_Score captured above all else -- include as many "
        "high-priority DCs as the hard constraints allow, even if it means using most of the "
        "distance/time budget to do it."
    ),
    "distance_focused": (
        "Prioritize minimizing total round-trip distance -- prefer the tightest geographic "
        "cluster of DCs available, even if it means capturing somewhat less total priority than "
        "a farther-spread combination would."
    ),
    "time_focused": (
